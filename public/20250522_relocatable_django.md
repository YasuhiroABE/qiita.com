...
tags:
  ...
  - RelocatableApp
private: false
updated_at: '2026-07-10T09:25:18+09:00'
...
Reverse Proxyでは戻るデータの中身までは書き換えませんし、したとしてもバグが入り込むだけでしょう。

そのため完全に対応させるためにはアプリケーション側で相対パスを徹底するしかありません。

# RelocatableなWebアプリケーションとは

Webアプリケーションを作成する時にURLパス上のどこにでも配置できるように、あらかじめ仕組みを入れておくわけですが、フレームワークとして対応しているものは実はあまり多くありません。

代表的なものはJ2EE(Java2 Enterprise Edition)ですが、EARファイルにWARファイルを格納して適切に設定することが前提です。

Tomcatをコンテナとして利用している場合にはWARファイルだけを構成すれば良いで、このルールを適用しなくても自由に作成が可能です。

EARファイルのWEB-INF/application.xmlはオプションなので、WARファイルだけで動作させることができます。

実際にはEARで構成できるかどうかを問わない、特定のパスの元で単体で動作すれば十分という条件でWARファイルを作成する現場がほとんどではないでしょうか。

このためJ2EEなら大丈夫というわけでもありませんし、もはやSpringの前に使われている現場も少ないでしょうけれど、一応仕様としては十分に検討されて対応しています。

## Djangoで設定で反応するURLを変更する

Djangoに限らずLL言語では起動時に、基底となるパス(context_root, e.g.:/my-app)を追加するようなロジックを記述するだけで、ほぼほぼ対応できると思います。
...
## やっぱりReverse Proxyサーバーで対応できないのか

残念ながらブラウザからアクセスがあった時に、到達先のURLとRequest内容を調整するのが仕事なので、戻されるデータの内容を書き換えたりはしてくれません。

一応プラグインなどでコンテンツを書き換える実装はありますが広告の挿入などが想定される用途だと思います。

入力となる書き換え対象のデータ構造が想定できることが前提で十分にテストする必要があります。

機能な十分で汎用性はありますが、十分にテストしないとバグに悩まされることになるでしょう。

SquidなどのFrontend Proxyサーバーにも組織内からのアクセスを禁止したい外部サイトを内部サイトに誘導するといった目的でレスポンスに含まれるURLを書き換える仕組みが備わっている場合があります。

これもセキュリティ向上などが目的で、書き換えをミスしても問題がないという前提で利用されるケースがほとんどだと思います。
