...
tags:
  ...
  - OIDC
private: false
updated_at: '2024-07-03T13:16:08+09:00'
...
詳細なテスト手順をまとめて、ansible roleとして[Github上で公開](https://github.com/YasuhiroABE/ansible-mediawiki-restoredb)しています。Ansible Galaxyにも登録しています。
...
2024/04/10に確認したところ、Ubuntu 24.04.2で新規にmysqldを動作させたタイミングでは正常に動作しなかったため、内部taskの動作順を変更しています。リファクタリング程度の変更のみで機能は追加されていません。
:::
...
本番環境ではMySQLのバックアップを保全しておけばリストアする必要はないので、mediawiki-1.43.0のtargzballを展開し、LocalSettings.phpファイルを配置した上で、maintenanceディレクトリで"$ sudo php update.php"を実行します。
...
1.35.9からの更新のタイミングで、ハードウェアをそれまでのPC Engines社製APUから、Protectli社製VP2420に変更しました。ハードウェアのコストが2倍になって、性能はPassmark値で6倍ほどになっています。SSDのIO性能にはほぼ変更がないので、体感できる性能差はそれほど大きくありません。やや反応が悪いくらいから、きびきび反応するようになったので変更した価値はありますが、6倍遅かったかというと、それほどではありません。

同時にUbuntu 20.04だったOSは、22.04を新規にインストールしていて、OSの初期設定には多くの変更作業が発生しています。interfacesファイルによるNIC定義からnetplanに変更されていたり様々な点が変わっています。
...
私の利用範囲では問題はまったくありませんでした。

Extensionsはきちんと1.43に対応したバージョンに置き換えないとmediawikiが動作しない問題があっただけです。
