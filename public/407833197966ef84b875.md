...
tags:
  ...
  - OIDC
private: false
updated_at: '2026-01-14T11:16:47+09:00'
...
* 1.35.8 to 1.39.0
  * Ubuntu 20.04.5 LTS amd64版 (作業完了後に22.04.1 LTSに更新)
* 1.35.9 to 1.39.3
...
手動で``sudo a2dismod php8.1 && sudo a2enmod php8.3``を実行してから、``sudo systemctl restart apache2.service``を実行しています。
:::
...
Mediawikiは長期サポートバージョンをLTSとしてリリースしており、私の環境ではこのLTS版のみを利用しています。
...
1.43.xは2024/12/21から2027/12までサポートされる予定のLTSバージョンです。

1.43.0では1.39.0のような大幅な変更がなかったため、2024年4月に1.39.11から1.43.0にアップグレードしました。

https://www.mediawiki.org/wiki/Version_lifecycle/ja

LTS版ではセキュリティ上の問題が頻繁に発見され、バージョンが更新されています。

同一LTSバージョン内の更新作業は比較的定型的な作業ですので、メーリングリストに入るなどして、頻繁に更新することをお勧めします。
...
テスト手順をまとめて、ansible roleとしてGithub上で公開(ansible-mediawiki-restoredb)しています。
