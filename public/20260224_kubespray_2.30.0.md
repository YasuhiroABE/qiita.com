---
title: Kubespray v2.30.0でクラスターをアップグレードした時のメモ
tags:
  - Ansible
  - kubernetes
  - kubespray
private: false
updated_at: '2026-08-03T14:05:41+09:00'
id: 44af14718c564cb1c19e
...
* Kubespray v2.29.1 (Kubernetes v1.33.7)

# 参考資料

https://qiita.com/YasuhiroABE/items/3a30fea863f9abb2475c
...
v2.30.0のリリースノードではCiliumなどについて対応が必要だとされていますが、特に対応は取っていません。
...
この時に**MAX UNAVAILABLE**が0などになっていないか、**MIN AVAILABLE**が1で正しいようにみえても、そのPodが1つのノードで全て稼動しているといったことがないように設定されているか確認が必要です。

自前のアプリケーションの場合は、Pod Anti Affinityの設定を見直してください。

設定されている例としてApache Zookeeperの設定を下記に掲載していますが、Nodeオブジェクトに設定しているlabels:にある``"kubernetes.io/hostname"``をみて、さらにPodのlabels:にある``app=zk``を含むPodをノードに重複しないように配置してくれます。

```yaml:ZookeeperのStatefulSetに設定されているPodAntiAffinity設定の抜粋
  template:
    metadata:
      labels:
        app: zk
...
$ git commit -a -m 'prepare to upgrade v2.30.0'
...
## 次のバージョンタグを確認する
$ git tag
## v2.28.xの次のv2.30.0からブランチを作成する
$ git checkout refs/tags/v2.30.0 -b t_v2.30.0
## 先頭の tag: v2.30.0 を確認する
$ git log
...
(k8s) $ git checkout t_v2.29.1 inventory/mycluster/inventory.ini
...
## 一つ前のバージョン(v2.29.1)との差分を確認する
(k8s) $ git diff t_v2.29.1 inventory/mycluster
...
(k8s) $ git diff t_v2.29.1 addons.yml
(k8s) $ vi addons.yml
(k8s) $ git diff t_v2.29.1 k8s-cluster.yml
(k8s) $ vi k8s-cluster.yml
(k8s) $ git diff t_v2.29.1 k8s-net-calico.yml
...
(k8s) $ git diff t_v2.29.1 ansible.cfg
## 変更したinventory.iniが前の版から変化していないことを確認する (v2.28.0以降は一つ前と比較する)
(k8s) $ git diff t_v2.29.1 inventory/mycluster/inventory.ini
...
## kube_version を確認するが、v2.29.1から設定ファイルにデフォルト値が記載されなくなったので、RELEASE_NOTESか、checksums.ymlファイルからサポートされているバージョンを確認する
...
(k8s) $ ansible-playbook upgrade-cluster.yml -b -i inventory/mycluster/inventory.ini -e kube_version=1.34.3
```
...
$ ansible-playbook upgrade-cluster.yml -b -i inventory/mycluster/inventory.ini -e kube_version=v1.34.3 --limit node4
...
この作業の間に不用意にansibleコマンドを実行するなどすると、最初に収集したfactsの内容が失なわれてエラーになる場合があります。

またここでは想定していない最初から``--limit``を指定して個別にクラスターをアップグレードしようとするのはエラーになるので、``docs/operations/upgrades.md``の指示に従ってください。

```bash:facts関連のエラーが発生した場合は次のコマンドを実行してから--limitを使う作業を実施する
$ ansible-playbook playbooks/facts.yml -b -i inventory/mycluster/inventory.ini
```

具体的な事例を次のセクションに記載しています。

## 今回の作業中に遭遇した問題への対応

今回は1つのクラスターでRook/CephのOperatorが自動的にpdbを作成してノードのdrainを阻止したことで、途中でローリングアップデートが停止しました。

```text:Nodeの1つでアップグレードが停止した状態
NAME    STATUS                     ROLES           AGE      VERSION
node1   Ready                      control-plane   5y38d    v1.34.3
node2   Ready                      control-plane   5y38d    v1.34.3
node3   Ready,SchedulingDisabled   <none>          5y38d    v1.33.7
node4   Ready                      <none>          4y312d   v1.34.3
node5   Ready                      <none>          4y312d   v1.34.3
```

この後に``--limit node3``を指定してNode単位でアップグレードしようとしたタイミングで問題が発生しました。

```text:ansible-playbookの実行中に停止した際のエラーメッセージ
An exception occurred during task execution. To see the full traceback, use -vvv. The error was: ansible.errors.AnsibleUndefinedVariable: 'ansible.vars.hostvars.HostVarsVars object' has no attribute 'main_access_ips'
```

``docs/operations/upgrades.md``に記載がありますが、ノード単位での操作を行う際には次のような指示があります。

1. facts.ymlをあらかじめ実行して全台の状態をキャッシュすること (実際のファイルの配置先は``playbooks/facts.yml``)
2. upgrade-cluster.ymlを``--limit kube_control_plane:etcd``を付けて実行する
3. upgrade-cluster.ymlで個別のノード名を``--limit``に指定して実行

これまでは最初に``--limit``を指定せずにansible-playbookを実行していたので問題なかったのですが、不用意にansibleコマンドを実行するなどしてキャッシュしてあったfactsの内容が失なわれたのだと思います。

```bash:実際のリカバリタスク
$ ansible-playbook playbooks/facts.yml -b -i inventory/mycluster/inventory.ini
$ ansible-playbook upgrade-cluster.yml -b -i inventory/mycluster/inventory.ini -e kube_version=1.34.3 --limit=node3
```

以上の操作で無事に作業を終えることができました。

### VIPが動作しない

あるクラスターではControl-PlaneのVIPを有効にしています。

```text:エラー
kube-vip-node1                             0/1     CrashLoopBackOff   7110 (3m16s ago)   4m4s
kube-vip-node2                             0/1     CrashLoopBackOff   7113 (15s ago)     3m59s
```

これはaddons.ymlファイルで必要な設定をOFFにしていたことが原因でした。

```yaml:修正前後のdiffの出力
diff --git a/inventory/mycluster/group_vars/k8s_cluster/addons.yml b/inventory/mycluster/group_vars/k8s_cluster/addons.yml
index f3e14431f..203685d99 100644
--- a/inventory/mycluster/group_vars/k8s_cluster/addons.yml
+++ b/inventory/mycluster/group_vars/k8s_cluster/addons.yml
@@ -223,8 +223,8 @@ argocd_enabled: false
...
+kube_vip_arp_enabled: true
+kube_vip_controlplane_enabled: true
 kube_vip_address: 192.168.110.20
 # loadbalancer_apiserver:
 #   address: "{{ kube_vip_address }}"
```

Control-Planeノードの/etc/kubernetes/manifests/kube-vip.ymlを確認すると、次のような設定がされていませんでした。
...
最終的にVIPだけを再設定することで、復旧させました。

```bash:復旧させるために実行したコマンド
$ cd kubernetes
$ . venv/k8s/bin/activate
(k8s) $ ansible kube_node -i inventory/mycluster/inventory.ini -m command -a 'uname -n'
(k8s) $ ansible-playbook upgrade-cluster.yml -b -i inventory/mycluster/inventory.ini -e kube_version=1.34.3 --tags=kube-vip
```
...
:::note
と思っていたのですが、最後にVIP関連でサービスを停止させるというミスを犯してしまいました。

addons.ymlのコメントアウトされている設定がデフォルト値だと思い込んでいて、変更しないなら有効にしなくて良いと考えたことが致命的な間違いでした。
:::
